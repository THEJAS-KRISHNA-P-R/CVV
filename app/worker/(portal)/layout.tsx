...
export default function WorkerLayout({ children }: { children: ReactNode }) {
  ...
  // ── Role guard: redirect non-workers immediately ───────────────────────────
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (!profile || !['worker', 'admin'].includes(profile.role)) {
        router.replace('/worker/login?error=not_a_worker')
        return
      }
      setAuthChecked(true)
    })()
  ...
  if (!authChecked) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-amber-950 to-stone-950 flex items-center justify-center">
